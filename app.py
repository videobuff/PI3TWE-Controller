...
SQLITE_MONITOR_ENABLED = os.environ.get("PI3TWE_SQLITE_MONITOR_ENABLED", "1") == "1"

# ---------------------
# Config
# ---------------------
DB_PATH = "/srv/pi3twe/app/pi3twe.db"
MONITOR_DB_PATH = "/srv/pi3twe/app/monitor.db"
...
def dht_init_once() -> None:
    """DHT disabled - using dht_logger.py instead"""
    pass
...
def _monitor_db_loop_DISABLED() -> None:
...
def start_monitor_db_logger_once_DISABLED() -> None:
...
def api_admin_calibration_get():
    admin_required()
    try:
        int_temp = float(setting_get("cal_int_temp_offset", "0.0"))
        int_hum = float(setting_get("cal_int_hum_offset", "0.0"))
        ext_temp = float(setting_get("cal_ext_temp_offset", "0.0"))
        ext_hum = float(setting_get("cal_ext_hum_offset", "0.0"))
    except Exception:
        int_temp = int_hum = ext_temp = ext_hum = 0.0
    
    # DHT data komt uit database via dht_logger.py
    it = read_monitor_latest(SRC_INT)
    ex = read_monitor_latest(SRC_EXT)
    
    return jsonify({
        "offsets": {"int_temp": int_temp, "int_hum": int_hum, "ext_temp": ext_temp, "ext_hum": ext_hum},
        "current_values": {"int": it or {}, "ext": ex or {}}
    })

@app.post("/api/admin/calibration")
def api_admin_calibration_set():
    admin_required()
    data = request.get_json(silent=True) or {}
    try:
        int_temp = float(data.get("int_temp", 0.0))
        int_hum = float(data.get("int_hum", 0.0))
        ext_temp = float(data.get("ext_temp", 0.0))
        ext_hum = float(data.get("ext_hum", 0.0))
    except Exception:
        abort(400, "Ongeldige calibratie waarden.")
    
    if abs(int_temp) > 10 or abs(ext_temp) > 10:
        abort(400, "Temp offset te groot (max ±10°C).")
    if abs(int_hum) > 20 or abs(ext_hum) > 20:
        abort(400, "Hum offset te groot (max ±20%).")
    
    setting_set("cal_int_temp_offset", str(int_temp))
    setting_set("cal_int_hum_offset", str(int_hum))
    setting_set("cal_ext_temp_offset", str(ext_temp))
    setting_set("cal_ext_hum_offset", str(ext_hum))
    audit("CALIBRATION_SET", current_user_id(), f"INT({int_temp:.1f}°C,{int_hum:.0f}%) EXT({ext_temp:.1f}°C,{ext_hum:.0f}%)")
    return jsonify({"ok": True})

@app.post("/api/admin/calibration/reset")
def api_admin_calibration_reset():
    admin_required()
    setting_set("cal_int_temp_offset", "0.0")
    setting_set("cal_int_hum_offset", "0.0")
    setting_set("cal_ext_temp_offset", "0.0")
    setting_set("cal_ext_hum_offset", "0.0")
    audit("CALIBRATION_RESET", current_user_id(), "All offsets reset to 0")
    return jsonify({"ok": True})
    if clear >= trip:
...
# DISABLED - now handled by dht-logger.service:         start_monitor_db_logger_once()
